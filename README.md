...
means you must download the data. Please follow these steps:
    ...
    4. Upon successfully downloading, extract the folder
...
You do not need to put the entire relative path when prompted, just the name of the file. 

We recommend using Menu Option 2 to peruse the smaller test files to find taxon to search using Menu Option 3. However, we do not recommend doing this
...
be prepared to terminate the program if it takes too long. Here are some randomly selected taxonomical entries (paths) that you can search for any componenent of:
...
Menu Option 3 will first the user to input the taxon they are locating. Then, it will ask the user which algorithm they would like to choose. After one is selected,
